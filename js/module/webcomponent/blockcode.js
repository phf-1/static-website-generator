...
`
    ...
    render() {
        const res = this._hihlighted === null ?  {content: html`<slot></slot>`, language: ""} : {content: html`${unsafeHTML(this._hihlighted.value)}`, language: this._hihlighted.language};
        return html`<pre><code data-highlighted="yes" class="hljs ${res.language}">${res.content}</code></pre>`;
    }

    firstUpdated(changedProperties) {
        const code = this.shadowRoot.querySelector("slot").assignedNodes()[0].wholeText.trim();
